          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <section className="col-span-12">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-[22px] font-bold tracking-tight">{lang === 'en' ? 'Browse by category' : 'Explorar por categoría'}</h2>
                <a className="text-[13px] font-semibold text-[#65A30D] hover:underline cursor-pointer" onClick={() => setActiveCat('')}>{lang === 'en' ? 'View all' : 'Ver todo'} →</a>
              </div>
              <div className="relative overflow-hidden -mx-4">
                <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 px-4 pr-8">
                {categoriesData.map(cat => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex-shrink-0 w-[105px] bg-white border rounded-2xl p-3 group cursor-pointer transition-all hover:shadow-md hover:border-[#84CC16]/40 ${isActive ? 'border-[#84CC16] ring-2 ring-[#84CC16]/20' : 'border-slate-200'}`}
                    >
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2.5 ${isActive ? 'bg-[#84CC16] text-white' : 'bg-[#84CC16]/20 text-[#65A30D]'}`}>
                        <Icon size={18} />
                      </div>
                      <h3 className="font-semibold text-[12px] line-clamp-2 text-slate-800 leading-tight">{cat.name?.[lang] || cat.name?.['es'] || cat.name}</h3>
                    </div>
                  );
                })}
                </div>
            ...
            <section className="col-span-12 mt-2">
              ...
              <div className="relative overflow-hidden -mx-4 lg:mx-0 px-4 lg:px-0">
