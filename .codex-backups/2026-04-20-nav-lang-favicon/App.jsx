...
const getImageUrl = (path, fallback = null) => {
  ...
  if (path.startsWith('[')) {
    try {
      ...
      if (arr && arr.length > 0) { const u = arr[0]; return (u.startsWith('http') || u.startsWith('data:')) ? u : `${STORAGE_URL}/${u}`; }
    } catch (e) {}
  }
...
const StaticPages = React.lazy(() => import('./components/screens/StaticPages'));
const AdDetailScreen = React.lazy(() => import('./components/screens/AdDetailScreen'));
const StorefrontScreen = React.lazy(() => import('./components/screens/StorefrontScreen'));


// --- MEXICAN STATES LIST ---
const MEXICO_STATES = [
  // Estados
  'Ciudad de Mexico','Jalisco','Nuevo Leon','Estado de Mexico','Puebla','Guanajuato',
  'Veracruz','Chihuahua','Baja California','Sonora','Coahuila','Tamaulipas','Sinaloa',
  'Oaxaca','Guerrero','Michoacan','San Luis Potosi','Hidalgo','Queretaro','Morelos',
  'Yucatan','Durango','Zacatecas','Aguascalientes','Tabasco','Nayarit','Quintana Roo',
  'Colima','Campeche','Tlaxcala','Baja California Sur','Chiapas',
  // CDMX Alcaldias
  ...
  // Jalisco
  'Guadalajara','Zapopan','Tlaquepaque','Tonala','Tlajomulco de Zuniga',
  'El Salto','Lagos de Moreno','Tepatitlan de Morelos','Ocotlan','Puerto Vallarta',
  'Chapala','Autlan de Navarro','Ciudad Guzman','Ameca',
  // Nuevo Leon
  'Monterrey','Guadalupe','San Nicolas de los Garza','Apodaca','Santa Catarina',
  'General Escobedo','Juarez','San Pedro Garza Garcia','Cadereyta Jimenez',
  'Garcia','Linares','Montemorelos','Sabinas Hidalgo','Allende',
  // Estado de Mexico
  'Ecatepec de Morelos','Naucalpan de Juarez','Toluca','Nezahualcoyotl',
  'Chimalhuacan','Tlalnepantla de Baz','Ixtapaluca','Tultitlan','Cuautitlan Izcalli',
  'Atizapan de Zaragoza','Nicolas Romero','Tecamac','Valle de Chalco','Texcoco',
  'Coacalco de Berriozabal','Chalco','Metepec','Lerma','Zinacantepec','Tenancingo',
  // Puebla
  'Puebla','San Andres Cholula','San Martin Texmelucan','Tehuacan','Atlixco',
  'Amozoc','Cuautlancingo','Huejotzingo','Teziutlan','Izucar de Matamoros',
  // Guanajuato
  'Leon','Irapuato','Celaya','Salamanca','Guanajuato','Silao','Penjamo',
  'Acambaro','Valle de Santiago','San Luis de la Paz','San Miguel de Allende',
  'Dolores Hidalgo','Morolean','Uriangato',
  // Veracruz
  'Veracruz','Xalapa','Coatzacoalcos','Cordoba','Orizaba','Poza Rica',
  'Minatitlan','Tuxpan','Boca del Rio','San Andres Tuxtla','Papantla',
  'Cosamaloapan','Tierra Blanca','Martinez de la Torre','Acayucan',
  // Chihuahua
  'Chihuahua','Ciudad Juarez','Delicias','Cuauhtemoc','Parral',
  'Nuevo Casas Grandes','Camargo','Jimenez','Guachochi',
  // Baja California
  'Tijuana','Mexicali','Ensenada','Tecate','Rosarito','San Felipe',
  // Sonora
  'Hermosillo','Ciudad Obregon','Nogales','Guaymas','San Luis Rio Colorado',
  'Navojoa','Agua Prieta','Puerto Penasco','Caborca','Empalme',
  // Coahuila
  'Saltillo','Torreon','Monclova','Piedras Negras','Acuna','Ramos Arizpe',
  'Frontera','San Pedro','Sabinas','Muzquiz',
  // Tamaulipas
  'Reynosa','Matamoros','Nuevo Laredo','Victoria','Tampico','Madero','Altamira',
  'Mante','Laredo (MX)','Rio Bravo',
  // Sinaloa
  'Culiacan','Mazatlan','Los Mochis','Ahome','Guasave','Navolato',
  'Salvador Alvarado','Escuinapa',
  // Oaxaca
  'Oaxaca de Juarez','Salina Cruz','Juchitan','Tuxtepec','Huatulco',
  'Puerto Escondido','Pochutla','Miahuatlan de Porfirio Diaz',
  // Guerrero
  'Acapulco','Chilpancingo','Iguala','Zihuatanejo','Taxco','Coyuca de Benitez',
  'Tlapa de Comonfort','Petatlan',
  // Michoacan
  'Morelia','Uruapan','Zamora','Lazaro Cardenas','Apatzingan','Zitacuaro',
  'Patzcuaro','Sahuayo','Jacona','Cotija',
  // San Luis Potosi
  'San Luis Potosi','Ciudad Valles','Matehuala','Soledad de Graciano Sanchez',
  'Rioverde','Tamazunchale','Ebano',
  // Hidalgo
  'Pachuca','Tulancingo','Tula de Allende','Huejutla','Ixmiquilpan','Tizayuca',
  'Actopan','Tepeapulco',
  // Queretaro
  'Queretaro','San Juan del Rio','El Marques','Corregidora','Pedro Escobedo',
  // Morelos
  'Cuernavaca','Jiutepec','Cuautla','Temixco','Yautepec','Jojutla',
  // Yucatan
  'Merida','Valladolid','Progreso','Tizimin','Uman','Izamal','Ticul',
  // Quintana Roo
  'Cancun','Playa del Carmen','Tulum','Chetumal','Cozumel','Felipe Carrillo Puerto',
  'Bacalar','Coba','Puerto Morelos','Isla Mujeres',
  // Tabasco
  'Villahermosa','Cardenas','Macuspana','Comalcalco','Paraiso','Huimanguillo',
  // Nayarit
  'Tepic','Bahia de Banderas','Ixtlan del Rio','Compostela','Santiago Ixcuintla',
  // Durango
  'Durango','Gomez Palacio','Lerdo','Santiago Papasquiaro','El Salto',
  // Zacatecas
  'Zacatecas','Fresnillo','Guadalupe','Jerez','Calera',
  // Aguascalientes
  'Aguascalientes','Pabellon de Arteaga','Rincon de Romos','Calvillo','Jesus Maria',
  // Colima
  'Colima','Manzanillo','Tecoman','Villa de Alvarez','Armeria',
  // Campeche
  'Campeche','Ciudad del Carmen','Champotón','Escarcega','Palizada',
  // Tlaxcala
  'Tlaxcala','Apizaco','Huamantla','Chiautempan','Calpulalpan',
  // Baja California Sur
  'La Paz','Los Cabos','Cabo San Lucas','San Jose del Cabo','Loreto','Todos Santos',
  // Chiapas
  'Tuxtla Gutierrez','San Cristobal de las Casas','Tapachula','Comitan',
  'Palenque','Ocosingo','Tonala','Pichucalco',
];

// --- LOCATION DROPDOWN COMPONENT ---
function LocationDropdown({ value, onChange, onDetect, onSelect, onClose }) {
  ...
  const filtered = MEXICO_STATES.filter(s => s.toLowerCase().includes(query.toLowerCase()));
  return (
    <div ref={ref} className="absolute top-full left-0 mt-1 w-72 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 overflow-hidden">
      <div className="p-2 border-b border-slate-100">
        <input autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar estado o ciudad..." className="w-full px-3 py-1.5 text-sm bg-slate-50 rounded-lg outline-none border border-slate-200 focus:border-[#84CC16]" />
      </div>
      <button onClick={onDetect} className="w-full flex items-center gap-2 px-4 py-2.5 hover:bg-[#84CC16]/10 text-sm font-semibold text-[#65A30D] border-b border-slate-100">
        <MapPin size={14} /> Detectar mi ubicación
      </button>
      <div className="max-h-52 overflow-y-auto">
        {filtered.map(s => (
          <button key={s} onClick={() => onSelect(s)} className="w-full text-left px-4 py-2 hover:bg-slate-50 text-sm text-slate-700">{s}</button>
        ))}
      </div>
    </div>
  );
...
// --- HELPER: get category name by lang ---
const getCatName = (cat, lang) => {
  if (!cat) return '';
  if (typeof cat.name === 'string') return cat.name;
  return cat.name?.[lang] || cat.name?.es || cat.name?.en || cat.slug || '';
};
...
export default function App() {
  ...
  const [userRole, setUserRole] = useState((JSON.parse(localStorage.getItem('user')) || {}).role || 'individual');
  ...
  const [showMobileLocDropdown, setShowMobileLocDropdown] = React.useState(false);
  const [showMobileLocIcon, setShowMobileLocIcon] = React.useState(false);
  const mobileLocIconRef = React.useRef(null);
  const [detectedCity, setDetectedCity] = React.useState(localStorage.getItem('userCity') || '');

  // Auto-detect location on first visit
  React.useEffect(() => {
    if (!localStorage.getItem('userCity') && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(pos => {
        fetch(`https://nominatim.openstreetmap.org/reverse?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&format=json&accept-language=es`)
          .then(r => r.json()).then(data => {
            const city = data.address?.city || data.address?.town || data.address?.municipality || data.address?.county || data.address?.suburb || data.address?.state || '';
            if (city) { setDetectedCity(city); localStorage.setItem('userCity', city); }
          }).catch(() => {});
      }, () => {});
    }
  }, []);
  ...
  const handleDetectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      fetch(`https://nominatim.openstreetmap.org/reverse?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&format=json&accept-language=es`)
        .then(r => r.json()).then(data => {
          const city = data.address?.city || data.address?.town || data.address?.municipality || data.address?.county || data.address?.suburb || data.address?.state || '';
          if (city) { setDetectedCity(city); setSearchLocationInput(city); setSelectedState(city); localStorage.setItem('userCity', city); setShowLocationDropdown(false); setShowMobileLocDropdown(false); }
        }).catch(() => {});
    }, () => alert('No se pudo detectar la ubicación'));
  };

  const handleSelectState = (state) => { setSelectedState(state); setSearchLocationInput(state); setDetectedCity(state); localStorage.setItem('userCity', state); setShowLocationDropdown(false); setShowMobileLocDropdown(false); };
  ...
  const executeSearch = useCallback(() => {
    ...
    setViewedAd(null);
    setViewedCompany(null);
  }, [searchQuery, searchLocationInput]);
  ...
  const renderStorefrontScreen = () => <StorefrontScreen company={viewedCompany} t={t} getImageUrl={getImageUrl} companyRatingStats={companyRatingStats} companyAds={companyAds} companyReviews={companyReviews} loadingCompanyAds={loadingCompanyAds} submittingReview={submittingReview} setShowUserReportModal={setShowUserReportModal} setViewedCompany={setViewedCompany} renderAdCard={renderAdCard} handleReviewSubmit={handleReviewSubmit} reviewForm={reviewForm} setReviewForm={setReviewForm} user={user} handleViewCompany={handleViewCompany} />;
  ...
  const renderTabBar = () => (
    <div className="md:hidden fixed bottom-0 w-full bg-white border-t border-gray-200 z-40 shadow-[0_-10px_20px_rgba(0,0,0,0.03)]">
      {/* Location + Language strip */}
      <div className="flex items-center justify-between px-4 pt-1.5 pb-0 text-[11px] text-slate-500">
        <button onClick={() => setShowMobileLocDropdown(true)} className="flex items-center gap-1 hover:text-[#65A30D]">
          <MapPin size={11} className="text-[#84CC16]"/>
          <span className="max-w-[140px] truncate">{searchLocationInput || detectedCity || 'Toda México'}</span>
        </button>
        <div className="flex items-center gap-1">
          <Globe size={11}/>
          <select value={lang} onChange={e => { setLang(e.target.value); localStorage.setItem('lang', e.target.value); }} className="bg-transparent text-[11px] font-bold outline-none cursor-pointer uppercase">
            {Object.keys(translations).map(l => <option key={l} value={l}>{l.toUpperCase()}</option>)}
          </select>
        </div>
      </div>
      <div className="pb-safe pt-1 px-6 flex justify-between items-center h-[68px]">
        <button onClick={() => { setCurrentTab('home'); navigate('/'); }} className={`flex flex-col items-center p-1 ${currentTab === 'home' ? 'text-[#84CC16]' : 'text-gray-400 hover:text-[#84CC16]'}`}>
          <Home className="w-6 h-6 mb-1" />
        </button>
        <button onClick={() => { setCurrentTab('home'); navigate('/'); window.scrollTo(0,0); setTimeout(() => mobileLocationInputRef.current?.focus(), 100); }} className="flex flex-col items-center p-1 text-gray-400 hover:text-[#84CC16]">
          <Search className="w-6 h-6 mb-1" />
        </button>
        <button onClick={() => { setCurrentTab('post'); navigate('/post'); }} className="flex flex-col items-center p-1 -mt-8">
          <div className="flex flex-col items-center justify-center bg-[#84CC16] text-white p-3.5 rounded-full shadow-lg border-4 border-[#f5f5f5]">
            <PlusCircle className="w-7 h-7" />
          </div>
        </button>
        <button onClick={() => { user ? setShowNotifications(!showNotifications) : setShowAuthModal(true); }} className={`flex flex-col items-center p-1 relative ${currentTab === 'notifications' ? 'text-[#84CC16]' : 'text-gray-400 hover:text-[#84CC16]'}`}>
        ...
        <button onClick={() => { user ? (setCurrentTab('profile'), navigate('/profile')) : setShowAuthModal(true); }} className={`flex flex-col items-center p-1 ${currentTab === 'profile' ? 'text-[#84CC16]' : 'text-gray-400 hover:text-[#84CC16]'}`}>
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-xl border-b border-slate-200">
        <div className="max-w-[1440px] mx-auto px-4 lg:px-6">
          <div className="flex items-center gap-3 h-[68px]">
            <a href="#" onClick={(e) => { e.preventDefault(); setCurrentTab('home'); navigate('/'); setViewedAd(null); setViewedCompany(null); setActiveCat(''); setSearchQuery(''); }} className="flex items-center gap-2.5 shrink-0 hover:opacity-90 transition-opacity">
              <MercastoLogo />
            </a>
            <div className="hidden lg:flex flex-1 items-center">
              ...
              <div className="relative flex items-center w-full max-w-[200px]">
                <input ref={desktopLocationInputRef} value={searchLocationInput} onChange={(e) => { setSearchLocationInput(e.target.value); if(e.target.value === '') { setSearchLocation(null); setSelectedState(''); } }} onFocus={() => setShowLocationDropdown(true)} onKeyDown={e => { if(e.key === 'Enter') { setShowLocationDropdown(false); executeSearch(); } }} placeholder={detectedCity || 'Estado o ciudad'} className="w-full px-2 py-2.5 bg-transparent outline-none text-[14px]" />
                {showLocationDropdown && <LocationDropdown value={searchLocationInput} onChange={setSearchLocationInput} onDetect={handleDetectLocation} onSelect={handleSelectState} onClose={() => setShowLocationDropdown(false)} />}
              </div>
                <div className="h-7 w-px bg-slate-200"></div>
            ...
            <div className="flex items-center gap-1 ml-auto">
              ...
              <div className="lg:hidden relative" ref={mobileLocIconRef}>
                <button
                  onClick={() => setShowMobileLocDropdown(v => !v)}
                  className="flex items-center gap-1 bg-slate-100/50 px-2 py-1 rounded-lg border border-slate-200/50 text-slate-600 hover:text-[#65A30D] hover:border-[#84CC16]/40 transition-colors"
                >
                  <MapPin className="w-3.5 h-3.5" />
                  {(searchLocationInput || detectedCity) && (
                    <span className="text-[11px] font-semibold max-w-[70px] truncate">{searchLocationInput || detectedCity}</span>
                  )}
                </button>
                {showMobileLocDropdown && (
                  <div className="absolute top-full right-0 mt-1 z-50">
                    <LocationDropdown
                      value={searchLocationInput}
                      onChange={setSearchLocationInput}
                      onDetect={handleDetectLocation}
                      onSelect={handleSelectState}
                      onClose={() => setShowMobileLocDropdown(false)}
                    />
                  </div>
                )}
              </div>
              ...
              <button onClick={() => { user ? (setCurrentTab('profile'), navigate('/profile')) : setShowAuthModal(true); setDashboardTab('favorites'); }} className="relative p-2.5 hover:bg-slate-100 rounded-xl">
              ...
              <button onClick={() => { user ? (setCurrentTab('profile'), navigate('/profile')) : setShowAuthModal(true); setViewedAd(null); setViewedCompany(null); }} className="flex items-center gap-2 pl-1 pr-2.5 py-1 hover:bg-slate-100 rounded-xl">
                {user?.avatar_url ? (
              ...
              <button onClick={() => { setCurrentTab('post'); navigate('/post'); setViewedAd(null); setViewedCompany(null); }} className="btn-lg bg-[#84CC16] hover:bg-[#65A30D] text-white shadow-md shadow-[#84CC16]/20 ml-1 hidden sm:inline-flex items-center gap-1.5">
