...
if count != 1:
    raise RuntimeError('canned comment data anchor not found')

# 4. Do not publish false Product/Offer schema for catalog references.
...
# 5. Catalog disclosure and seller-acquisition CTA.
title = '            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-4 leading-tight">{localizedText(ad.title, lang)}</h1>'
...
# 6. Show rating only for genuine review data and never for catalog references.
pattern = (
...
""")
content, count = re.subn(pattern, lambda _: rating_row, content, count=1, flags=re.S | re.M)
if count != 1:
    raise RuntimeError('rating display anchor not found')

# 7. Hide false recency/view social signals for catalog references.
...
if calendar not in content or views not in content:
    raise RuntimeError('date/views metadata anchors not found')
content = content.replace(calendar, '              {!isCatalogFiller && <span className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900 px-3 py-2 rounded-xl"><Calendar size={16}/> {new Date(ad.created_at).toLocaleDateString()}</span>}', 1)
content = content.replace(views, '              {!isCatalogFiller && <span className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900 px-3 py-2 rounded-xl"><BarChart3 size={16}/> {ad.views || 0} vistas</span>}', 1)

# 8. Remove fake testimonials and the nonfunctional review form.
...
if start < 0 or end < 0:
    raise RuntimeError('review section boundaries not found')
content = content[:start] + content[end:]

path.write_text(content)
print('Catalog detail finalized without synthetic activity.')
