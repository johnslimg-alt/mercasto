...
curl "${CURL_COMMON[@]}" -c "$TMP_DIR/cookies" -o /dev/null \
...
cookie_status="$(curl "${CURL_COMMON[@]}" -o "$TMP_DIR/cookie-body" -w '%{http_code}' \
...
curl "${CURL_COMMON[@]}" -D "$TMP_DIR/untrusted-headers" -o /dev/null \
...
curl "${CURL_COMMON[@]}" -D "$TMP_DIR/trusted-headers" -o /dev/null \
  -X OPTIONS "$BASE_URL/api/user/profile" \
  -H 'Origin: https://mercasto.com' \
  -H 'Access-Control-Request-Method: PUT' \
  -H 'Access-Control-Request-Headers: authorization,content-type'
