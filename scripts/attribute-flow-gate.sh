...
PUBLIC_ATTRIBUTE_UTIL="src/utils/publicListingAttributes.js"
PUBLIC_ATTRIBUTE_TEST="tests/public-listing-attributes.test.mjs"
...
test -f "$PUBLIC_ATTRIBUTE_UTIL"
test -f "$PUBLIC_ATTRIBUTE_TEST"
test -f "$CONTROLLER"
...
grep -qF "publicListingAttributeEntries(attributes)" "$DETAIL_SCREEN"
grep -qF "publicAttributeEntries.map" "$DETAIL_SCREEN"
grep -qF "fieldDef.label" "$DETAIL_SCREEN"
grep -qF "INTERNAL_ATTRIBUTE_PREFIXES" "$PUBLIC_ATTRIBUTE_UTIL"
grep -qF "catalog_" "$PUBLIC_ATTRIBUTE_UTIL"
grep -qF "editorial_" "$PUBLIC_ATTRIBUTE_UTIL"
node --test "$PUBLIC_ATTRIBUTE_TEST" >/dev/null
