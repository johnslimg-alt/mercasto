...
grep -qF "private const MAX_AD_UPLOAD_BYTES = 60 * 1024 * 1024;" "$CONTROLLER"
grep -qF '$this->validateAggregateUploadSize($request);' "$CONTROLLER"
grep -qF "El tamaño total de fotos y video no puede superar 60 MB por solicitud." "$CONTROLLER"
