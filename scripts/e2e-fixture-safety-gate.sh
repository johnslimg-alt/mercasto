...
PROD_SELLER_E2E="tests/e2e/seller-production-smoke.spec.js"
...
test -f "$PROD_SELLER_E2E"
grep -qF 'Production seller read-only smoke' "$PROD_SELLER_E2E"
grep -qF 'request.get' "$PROD_SELLER_E2E"
if grep -Eiq 'request\.(post|put|patch|delete)|method:[[:space:]]*["\x27](POST|PUT|PATCH|DELETE)' "$PROD_SELLER_E2E"; then
  echo "FAIL: production seller smoke must remain read-only" >&2
  exit 1
fi
grep -qF 'seller-production-smoke.spec.js' "$WORKFLOW"
if grep -Eq 'auth-flow\.spec|ads-lifecycle\.spec|payments\.spec' "$WORKFLOW"; then
  echo "FAIL: mutating/full lifecycle specs must not run in the production seller workflow" >&2
  exit 1
fi
