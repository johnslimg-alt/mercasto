...
grep -qF "Route::middleware('throttle:sensitive-profile')->put('/user/password', [ProfileController::class, 'changePassword'])" "$ROUTES"
grep -qF "Route::middleware('throttle:sensitive-profile')->post('/user/password', [ProfileController::class, 'changePassword'])" "$ROUTES"
grep -qF "Route::middleware('throttle:sensitive-profile')->delete('/user', [AccountDeletionController::class, 'delete'])" "$ROUTES"
grep -qF "Route::middleware(['api', 'auth:sanctum', 'throttle:sensitive-profile'])->delete('/api/user', [AccountDeletionController::class, 'delete'])" backend/bootstrap/app.php
...
grep -qF "The SMS code is the first factor only" "$AUTH"
grep -qF "'two_factor' => true" "$AUTH"
phone_verify_block="$(sed -n '/const handlePhoneVerifySubmit = async/,/const resendVerificationEmail = async/p' "$APP")"
grep -qF "if (result.two_factor)" <<<"$phone_verify_block"
grep -qF "setTwoFactorChallengeToken(result.challenge_token || '')" <<<"$phone_verify_block"
grep -qF "setRequiresTwoFactor(true)" <<<"$phone_verify_block"
two_factor_submit_block="$(sed -n '/const handleTwoFactorSubmit = async/,/const handlePhoneRequestSubmit = async/p' "$APP")"
grep -qF "setUserRole(result.user?.role || 'individual')" <<<"$two_factor_submit_block"
...
grep -q -E "allowedProviders = \['google', 'apple', 'telegram'(, 'twitter')?\]" "$AUTH"
