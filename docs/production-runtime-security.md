...
Production Laravel sessions are encrypted. `SESSION_ENCRYPT=true` is the documented setting; if the variable is omitted, `backend/config/session.php` derives the safe default directly from `APP_ENV` during configuration bootstrap and enables encryption for production. An explicit `SESSION_ENCRYPT=false` fails the production readiness smoke.
