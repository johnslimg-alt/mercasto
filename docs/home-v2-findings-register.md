...
| 7 | **The custom language menu is enabled on one route only.** `useCustomLanguageMenu` is keyed to `/design-v2`; `/`, `/listings` and the ad-detail route still render `desktop-language-select`. **Correction:** I first called this "a one-line condition change". It is not. Four specs drive the native select — `header-geometry`, `mobile-shell-touch-targets`, `archived-language-fallback`, and `production-i18n-qa`, the last of which asserts on `/` that the native select contains exactly the 11 active language codes and excludes `he`/`yi`. Flipping the condition on `/` would break the 11-language production QA. Per-option testids (`language-option-<code>`) are now in place to make the test migration mechanical; the migration itself is a scoped follow-up, not a cutover blocker. | Medium (scoped follow-up) | `App.jsx` + 4 specs | `docs/home-v2-language-selector-audit.md` |
