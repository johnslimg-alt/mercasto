...
| --- | --- | --- | --- | --- |
| 4 | **Featured fetch ignores the configured API base.** `fetch('/api/ads/featured')` drops the `window.__API_URL__` / env resolution the rest of the app uses, and ignores `window.__FEATURED_ADS_PROMISE__`, so the server-side prefetch is wasted and an above-the-fold request is added. | Medium | `HomeScreenV2.jsx` | `docs/home-v2-real-data-audit.md` |
| 5 | **No filter analytics event exists anywhere in `src/`.** Opening the V2 filter panel emits only the generic `ui_click`, so §13's "filter opened / filter applied" cannot be reported for any surface. | Medium | shared analytics | `docs/home-v2-analytics-audit.md` |
| 6 | **`favoriteAdded` has zero call sites.** The helper is defined and mapped into the Meta/TikTok bridges, but `handleToggleFavorite` emits nothing, so favourites are invisible for signed-in users. | Medium | shared analytics | `docs/home-v2-analytics-audit.md` |
