...
function suggestionPayload({ lowConfidence = false } = {}) {
  ...
  return {
    available: true,
    suggestions: {
      category: lowConfidence ? null : { value: 'motor', confidence },
      subcategory: lowConfidence ? null : { value: 'Sedán', confidence },
      attributes: lowConfidence ? {} : { marca: { value: 'Nissan', confidence: 0.9 } },
      title: lowConfidence ? null : { value: 'Nissan Versa usado', confidence },
      description: lowConfidence ? null : { value: 'Nissan Versa usado en buen estado visible.', confidence },
    },
    runtime: {
      provider: 'ollama',
...
async function prepare(page, scenario = 'ok') {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (pathname.endsWith('/api/ads/autofill') && request.method() === 'POST') {
...
test('text-only autofill stays suggestion-only until seller applies fields', async ({ page }, testInfo) => {
  ...
  expect(requests[0]).toContain('name="hint_text"');
...
test('photo-only and mixed autofill send only new seller media', async ({ page }, testInfo) => {
  ...
  expect(requests[0]).toContain('name="images[]"');
...
test('empty low-confidence proposal exposes no apply action', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'chromium');
...
test('gateway failure leaves the manual publish flow usable on mobile', async ({ page }, testInfo) => {
  ...
  await page.getByTestId('listing-autofill-run').click();
  await expect(page.getByText(/Suggestions are unavailable/i)).toBeVisible();
