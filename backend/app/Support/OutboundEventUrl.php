 ...
 * place.
 *
 * Note on `app.frontend_url`: Laravel's base config defaults it to
 * `http://localhost:3000` (vendor/laravel/framework/config/app.php) and this
 * repository's deployment configuration never sets `FRONTEND_URL`. Rewriting a
 * valid page URL to a configured "canonical" origin would therefore be able to
 * replace real page URLs with localhost:3000 in production. This helper keeps the
 * candidate's own origin instead, and only falls back to configuration when there
 * is no usable URL at all.
 *
