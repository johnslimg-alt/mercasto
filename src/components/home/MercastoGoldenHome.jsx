...
export default function MercastoGoldenHome({serverAds=[],featuredAds=[],executeSearch,setSearchQuery,setActiveCat,setSearchLocationInput,setSelectedState,setAuthMode,setShowAuthModal,selectedState='',searchLocationInput='',openPricing,handleViewAd,getImageUrl,lang='es',t={},user,unreadCount=0}){
 ...
 <h1 className="mcg-responsive-h1"><span className="mcg-h1-desktop">Cosas increíbles<br/>más cerca de ti</span><span className="mcg-h1-tablet">Encuentra, compra y vende</span><span className="mcg-h1-mobile">Encuentra cerca de ti</span></h1>
...
function DeviceLayouts({q,setQ,search,searchTerm,category,publish,nav,openAi,ads,lang,getImageUrl,handleViewAd,account,notifications,unreadCount,t}){
 ...
 return <><div className="mcg-tablet"><div className="mcg-intro"><div className="mcg-tablet-title-spacer" aria-hidden="true"/><p>Un México con más oportunidades para todos</p></div><div className="mcg-dsearch"><div><Icon name="search"/><input data-testid="golden-tablet-search-input" value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&search()} placeholder="Qué estás buscando?"/></div><button data-testid="golden-tablet-filter" onClick={()=>nav('/listings')}><Icon name="filter"/>Filtros</button></div></div>
 <div className="mcg-mobile"><div className="mcg-msearch"><div><Icon name="search"/><input data-testid="golden-mobile-search-input" value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&search()} placeholder="Buscar en Mercasto..." aria-label="Buscar en Mercasto"/></div><button type="button" data-testid="golden-mobile-search-submit" className="mcg-msearch-go" aria-label="Buscar" onClick={search}><Icon name="search"/></button><button data-testid="golden-mobile-filter" aria-label="Filtros" onClick={()=>nav('/listings')}><Icon name="filter"/></button></div><section className="mcg-mobile-ai"><span className="mcg-ai-badge">Mercasto AI <small>BETA</small></span><h2>Tu asistente para <em>comprar, vender</em> y publicar.</h2><div><Icon name="bot" size={56}/></div></section><section className="mcg-mobile-chat"><Icon name="bot"/><p><b>Mercasto AI</b><br/>Hola! En qué te puedo ayudar hoy?</p><button onClick={()=>openAi?.()}><Icon name="right"/></button></section><div className="mcg-mactions">{actions.map((a,i)=><button key={a[1]} onClick={i===2?publish:i===0?search:()=>nav('/listings')}><span><Icon name={a[0]}/></span><b>{a[1]}</b><small>{a[2]}</small></button>)}</div><DeviceCategories category={category}/><DeviceAds ads={ads} lang={lang} getImageUrl={getImageUrl} handleViewAd={handleViewAd}/></div>
