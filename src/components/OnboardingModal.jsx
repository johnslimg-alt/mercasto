...
const SLIDES = [
  {
    id: 'welcome',
    emoji: '🎉',
    title: '¡Bienvenido a Mercasto!',
    subtitle: 'El marketplace de confianza para México',
    content: (
      <div className="flex flex-col items-center gap-6 py-4">
        ...
        <div className="flex gap-3 flex-wrap justify-center">
          {['🚗 Autos', '🏠 Inmuebles', '📱 Electrónica', '💼 Empleo'].map(tag => (
    ...
    content: (
      <div className="flex flex-col gap-4 py-2 w-full max-w-xs mx-auto">
        {[
          ...
          { step: '2', icon: '💬', label: 'Chatea con compradores', desc: 'Recibe mensajes y negocia de forma segura' },
          { step: '3', icon: '✅', label: 'Vende seguro', desc: 'Coordina la entrega y recibe tu pago' },
        ].map(({ step, icon, label, desc }) => (
          <div key={step} className="flex items-start gap-4 p-3 bg-slate-50 rounded-2xl border border-slate-100">
  ...
  },
  {
    ...
    subtitle: 'Los vendedores verificados venden 3× más rápido',
    content: null, // rendered separately to allow navigation
  },
];

export default function OnboardingModal({ onClose, user }) {
  ...
  const checklist = [
    { icon: Camera, label: 'Agrega una foto de perfil', done: !!(user?.avatar_url || user?.profile_photo_path || user?.avatar) },
    { icon: Phone, label: 'Verifica tu teléfono', done: !!(user?.phone_verified) },
    { icon: FileText, label: 'Escribe una descripción', done: !!(user?.bio && user.bio.trim()) },
  ];
          ...
          {slide.id !== 'profile' ? (
            ...
            <div className="flex flex-col gap-3 py-2">
              <p className="text-slate-500 text-[13px] text-center mb-1">
                Completa estos pasos para generar más confianza con los compradores:
              </p>
