        ...
        return (
          <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group relative">
            <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
              {metric} {t.impressions || 'impresiones'} · {d.clicks || 0} {t.clicks || 'clics'}
            </div>
...
function WkTrend({ thisWeek, lastWeek, t }) {
  if (lastWeek === 0 && thisWeek === 0) return null;
  if (lastWeek === 0) return <span className="flex items-center gap-0.5 text-[11px] text-emerald-600 font-semibold"><ArrowUp className="w-3 h-3"/>{t.new_word || 'Nuevo'}</span>;
  const pct = Math.round(((thisWeek - lastWeek) / lastWeek) * 100);
  if (pct > 0)  return <span className="flex items-center gap-0.5 text-[11px] text-emerald-600 font-semibold"><ArrowUp className="w-3 h-3"/>+{pct}% {t.vs_last_week || 'vs semana pasada'}</span>;
  if (pct < 0)  return <span className="flex items-center gap-0.5 text-[11px] text-red-500 font-semibold"><ArrowDown className="w-3 h-3"/>{pct}% {t.vs_last_week || 'vs semana pasada'}</span>;
  return <span className="flex items-center gap-0.5 text-[11px] text-slate-400 font-semibold"><Minus className="w-3 h-3"/>{t.no_change || 'Sin cambio'}</span>;
}
...
export default function SellerStatsScreen({ token }) {
  ...
  const STATUS_LABEL = {
    ...
    pending: t.pending_status || 'Pendiente',
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t.load_stats_failed || 'Error al cargar estadísticas');
  ...
  if (error) return (
    <div className="text-center py-16 text-slate-500">
      ...
      <button onClick={load} className="mt-4 text-sm text-[#65A30D] underline">{t.retry_btn || 'Reintentar'}</button>
    </div>
  );
  ...
  return (
    <div className="space-y-6 text-slate-900 dark:text-white">
      <div>
        <h2 className="text-[18px] font-bold text-slate-900 dark:text-white">{t.stats || 'Estadísticas'}</h2>
        <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-0.5">{t.stats_performance_subtitle || 'Rendimiento de tus anuncios en los últimos 7 días'}</p>
      </div>

      {/* KPI row */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiCard icon={Eye}           label={t.impressions || 'Impresiones'}     value={fmt(data.total_impressions)}     color="bg-blue-500"
          sub={<WkTrend thisWeek={data.views_this_week} lastWeek={data.views_last_week} t={t} />} />
        <KpiCard icon={MousePointerClick} label={t.clicks || 'Clics'}       value={fmt(data.total_clicks)} color="bg-[#84CC16]" sub={`CTR ${data.ctr || 0}%`} />
        <KpiCard icon={Megaphone}     label={t.promoted || 'Promocionados'}   value={fmt(data.active_promoted_ads)}  color="bg-amber-500" />
        <KpiCard icon={Heart}         label={t.favorites || 'Favoritos'}        value={fmt(data.total_favorites || data.saved_ads || 0)} color="bg-rose-500" />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm dark:shadow-none">
          <div className="text-[12px] text-slate-500 dark:text-slate-400 font-medium mb-1">{t.active_ads || 'Anuncios Activos'}</div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm dark:shadow-none">
          <div className="text-[12px] text-slate-500 dark:text-slate-400 font-medium mb-1 flex items-center gap-1"><CreditCard className="w-3.5 h-3.5"/>{t.promo_investment || 'Inversión promo'}</div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm dark:shadow-none col-span-2 md:col-span-1">
          <div className="text-[12px] text-slate-500 dark:text-slate-400 font-medium mb-1">{t.cpc || 'Costo por clic'}</div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm dark:shadow-none">
        <h3 className="text-[14px] font-semibold text-slate-800 dark:text-white mb-4">{t.impressions_clicks_7_days || 'Impresiones y clics — últimos 7 días'}</h3>
        <SparkBar data={data.views_by_day} t={t} />
      </div>
      ...
      {data.top_ads && data.top_ads.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800">
            <h3 className="text-[14px] font-semibold text-slate-800 dark:text-white">{t.top_ads || 'Top Anuncios'}</h3>
          </div>
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 text-[11px] uppercase tracking-wide">
                  <th className="text-left px-5 py-2.5 font-semibold">{t.title || 'Título'}</th>
                  <th className="text-right px-4 py-2.5 font-semibold">{t.imp_short || 'Imp.'}</th>
                  <th className="text-right px-4 py-2.5 font-semibold">{t.clicks || 'Clics'}</th>
                  <th className="text-right px-4 py-2.5 font-semibold">{t.ctr || 'CTR'}</th>
                  <th className="text-right px-4 py-2.5 font-semibold">{t.favorites_short || 'Favs'}</th>
                  <th className="text-right px-4 py-2.5 font-semibold">{t.cpc || 'CPC'}</th>
                  <th className="text-right px-5 py-2.5 font-semibold">{t.status || 'Estado'}</th>
                </tr>
              </thead>
