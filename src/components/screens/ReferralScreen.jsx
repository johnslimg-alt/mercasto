...
export default function ReferralScreen() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/referral`, {
      ...
      .then(r => {
        if (!r.ok) throw new Error('No se pudo cargar el programa de referidos');
        return r.json();
      })
      .then(setData)
      .catch(() => setError('No pudimos cargar tu enlace de referido. Inténtalo de nuevo.'));
  }, [navigate]);
  ...
  const handleWhatsApp = () => {
    if (!data) return;
    const msg = encodeURIComponent(`¡Únete a Mercasto y compra o vende en México! Regístrate con mi enlace: ${data.referral_url}`);
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 pt-8 sm:pt-10 text-slate-900 dark:text-white">
      ...
      <p className="text-slate-500 dark:text-slate-300 mb-6">
        Cada amigo que se registre con tu enlace te da <strong>1 destacado gratis</strong> para tus anuncios.
      </p>

      {/* Referral Link */}
      <div className="bg-lime-50 border border-lime-200 rounded-2xl p-4 mb-6 dark:bg-slate-900 dark:border-lime-400/30">
        <p className="text-sm text-slate-500 dark:text-slate-300 mb-1">Tu enlace de referido</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 text-center shadow-sm dark:bg-slate-900 dark:border-slate-800">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-300 mt-1">Amigos invitados</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-2xl p-4 text-center shadow-sm dark:bg-slate-900 dark:border-slate-800">
          <p className="text-3xl font-bold text-yellow-500">{data.credits}</p>
          <p className="text-sm text-slate-500 dark:text-slate-300 mt-1">Créditos ganados</p>
        </div>
      </div>
      ...
      {history.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold mb-3">Historial de referidos</h2>
      ...
      {history.length === 0 && (
        <div className="text-center py-8 text-slate-400 dark:text-slate-500">
          <p>Aún no has invitado a nadie. ¡Comparte tu enlace!</p>
        </div>
      )}
    </div>
  );
