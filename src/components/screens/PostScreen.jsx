...
export default function PostScreen({ categoriesData, debouncedLocation, editingAd, form, handleImageChange, handlePostSubmit, images, isMapUpdating, lang, postLoading, removeImage, removeImageById, reorderImages, setEditingAd, setForm, setVideoFile, t, videoFile, aiLoading, handleGenerateDescription }) {
    ...
    const categoryFields = React.useMemo(() => {
      ...
      // Create a map of normalized api fields
      const apiFieldsMap = {};
      apiFields.forEach(f => {
        const rawId = f.id || f.key;
        const normalizedId = aliases[rawId] ?? rawId;
        apiFieldsMap[normalizedId] = { ...f, normalizedId };
      });

      // Build merged fields starting with localFields
      const mergedFields = localFields.map(localField => {
        const localId = localField.id || localField.key;
        const apiField = apiFieldsMap[localId];
        ...
        if (apiField) {
          return {
            ...
            id: localId,
            key: localId,
            required: apiField.required ?? localField.required,
            type: localField.type || apiField.type,
            options: localField.options || apiField.options,
          };
        }
        return localField;
      });

      // Append any apiFields that were not merged
      apiFields.forEach(f => {
        const rawId = f.id || f.key;
        const normalizedId = aliases[rawId] ?? rawId;
        const alreadyMerged = localFields.some(lf => (lf.id || lf.key) === normalizedId);
        if (!alreadyMerged) {
          mergedFields.push({
            ...f,
            id: normalizedId,
      ...
      // Filter out subcategory since it is selected in step 1
      return mergedFields.filter(f => (f.id || f.key) !== 'subcategory');
                    ...
                    <div className="border border-slate-200 dark:border-slate-800 rounded-2xl p-5 bg-slate-50/50 dark:bg-slate-950/50">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {categoryFields.map(field => {
                          ...
                          if (fieldId === 'modelo') {
                            const selectedBrand = (form.attributes?.marca || '').toLowerCase();
                            const availableModels = brandModelsMap[selectedBrand] || [];
                            const isBrandSelected = !!selectedBrand;

                            return (
                              <div key={fieldId} className="space-y-2 col-span-1">
                                <label className="block text-[13px] font-semibold text-slate-700 dark:text-slate-300">{field.label}</label>
                                <select
                                  ...
                                  className="w-full px-3.5 py-2.5 border border-slate-300 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-[#84CC16]/30 focus:border-[#84CC16] text-[14px] bg-white dark:bg-slate-950 text-slate-900 dark:text-white cursor-pointer transition-all disabled:opacity-50"
                                >
                                  {!isBrandSelected ? (
                          ...
                          if (fieldId === 'year' || fieldId === 'ano' || fieldId === 'año') {
                            const years = [];
                          ...
                          if (field.type === 'checkbox' && (field.options || []).length > 2) {
                            return (
                              <div key={fieldId} className="col-span-full border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 bg-white dark:bg-slate-950/40">
                                <label className="block text-[13px] font-semibold text-slate-700 dark:text-slate-300 mb-2.5">{field.label}</label>
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                                  {(field.options || []).map(opt => {
                                    const val = typeof opt === 'object' ? opt.value : opt;
                                    const label = typeof opt === 'object' ? (opt.label?.[lang] || opt.label?.['es'] || opt.value) : opt;
                                    const currentValues = (form.attributes?.[fieldId] || '').split(',').map(s => s.trim()).filter(Boolean);
                                    const isChecked = currentValues.includes(val);
                                    return (
                                      <label key={val} className="flex items-center gap-2 text-[13px] font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                                        <input
                                          type="checkbox"
                                          checked={isChecked}
                                          onChange={e => {
              ...
              {currentStep === 3 && (
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div>
                     ...
                     {/* maps.google.com/maps fallback check */}
                  </div>
