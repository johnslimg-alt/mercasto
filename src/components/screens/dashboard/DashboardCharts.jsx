...
import { getDashboardChartCopy } from '../../../constants/dashboardChartCopy';
...
export default function DashboardCharts({ analyticsData, categoryStats, isDarkMode }) {
  ...
  const chartCopy = getDashboardChartCopy(lang);

  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-8 mt-4">
      <div className="w-full">
        <h4 className="text-[13px] font-bold text-slate-400 uppercase tracking-wider mb-6 flex items-center gap-2"><BarChart3 className="w-4 h-4"/> {t.views}</h4>
          ...
          {analyticsData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={analyticsData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                ...
                <Tooltip content={<ChartTooltip unit={t.views} isDarkMode={isDarkMode} />} cursor={{ stroke: isDarkMode ? '#64748B' : '#94A3B8', strokeWidth: 1, strokeDasharray: '3 3' }} />
            ...
            <div className="w-full h-full flex items-center justify-center text-slate-400 text-[13px] font-medium">{t.no_results_found}</div>
          )}
        </div>
      </div>

      <div className="w-full">
        <h4 className="text-[13px] font-bold text-slate-400 uppercase tracking-wider mb-6 flex items-center gap-2"><QrCode className="w-4 h-4"/> {t.contacts_qr}</h4>
        <div className="h-56 w-full">
          {analyticsData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={analyticsData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: isDarkMode ? '#94A3B8' : '#64748B', fontSize: 11, fontWeight: 500 }} tickMargin={10} minTickGap={20} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: isDarkMode ? '#94A3B8' : '#64748B', fontSize: 11, fontWeight: 500 }} allowDecimals={false} />
                <Tooltip content={<ChartTooltip unit={t.clicks} isDarkMode={isDarkMode} />} cursor={{ stroke: isDarkMode ? '#64748B' : '#94A3B8', strokeWidth: 1, strokeDasharray: '3 3' }} />
      ...
      <div className="w-full">
        <h4 className="text-[13px] font-bold text-slate-400 uppercase tracking-wider mb-6 flex items-center gap-2"><PieChartIcon className="w-4 h-4"/> {t.categories}</h4>
        <div className="h-56 w-full">
                  ...
                  if (active && payload && payload.length) {
                    return (
                      <div className={`text-center px-3 py-2 rounded-xl shadow-xl border ${isDarkMode ? 'bg-slate-800 border-slate-600 text-slate-100' : 'bg-slate-900 border-slate-700 text-white'}`}>
                        ...
                        <div className="text-[12px] font-bold">{payload[0].value} {payload[0].value === 1 ? chartCopy.ad_singular : t.ads}</div>
                      </div>
                    );
                  }
                  return null;
