...
export default function ListingAutofillPanel({
  ...
  const run = async () => {
    ...
    await analyze({ shortText, images: newPhotos, locale: lang });
  };

  return (
    <section className="mx-auto mb-4 w-full max-w-3xl rounded-2xl border border-lime-200 bg-gradient-to-br from-lime-50 to-white p-4 shadow-sm dark:border-lime-500/20 dark:from-lime-500/10 dark:to-slate-950" data-testid="listing-autofill-panel">
      <div className="flex items-start gap-3">
        <div className="mt-0.5 rounded-xl bg-lime-500 p-2 text-slate-950"><WandSparkles size={18} /></div>
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        ...
        />
        <button type="button" data-testid="listing-autofill-run" onClick={run} disabled={loading || !canAnalyze} className="min-h-11 rounded-xl bg-slate-950 px-4 text-xs font-extrabold text-white hover:bg-black disabled:cursor-not-allowed disabled:opacity-40 dark:bg-lime-500 dark:text-slate-950 dark:hover:bg-lime-400">
      ...
      {error && <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:bg-amber-500/10 dark:text-amber-200">{copy.unavailable}</p>}
      ...
      {suggestions && (
        <div className="mt-4 space-y-2" data-testid="listing-autofill-suggestions">
          ...
          <SuggestionRow label={copy.subcategory} suggestion={suggestions.subcategory_hint} copy={copy} disabled={!category} onApply={() => onApplySubcategory?.(category, suggestions.subcategory_hint?.value)} />
            ...
            <div className="pt-1">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {Object.entries(suggestions.attributes).map(([key, suggestion]) => (
                  <SuggestionRow key={key} label={key} suggestion={suggestion} copy={copy} disabled={form.category !== category} onApply={() => onApplyAttribute?.(category, key, suggestion.value)} />
                ))}
              </div>
            </div>
          )}
      ...
      <div className="mt-3 flex items-center gap-1.5 text-[10px] font-semibold text-slate-500 dark:text-slate-400">
        <ShieldCheck size={13} className="text-lime-600" /> {copy.privacy}
      </div>
    </section>
  );
}
