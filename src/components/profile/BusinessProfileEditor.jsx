    ...
    <form onSubmit={handleSave} className="bg-white rounded-2xl p-6 shadow-sm space-y-5 border border-slate-100">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input value={form.business_name} onChange={event => updateField('business_name', event.target.value)} maxLength={120} className="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-lime-400" placeholder="Mercasto Autos Veracruz" />
        </div>
        <div>
          <label className="block text-xs font-medium text-slate-500 mb-1">RFC</label>
          <input value={form.business_rfc} onChange={event => updateField('business_rfc', event.target.value.toUpperCase())} maxLength={13} className="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-lime-400" placeholder="XAXX010101000" />
      ...
      <div>
        ...
        <textarea value={form.business_description} onChange={event => updateField('business_description', event.target.value)} maxLength={1200} rows={4} className="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-lime-400" placeholder="Describe qué vendes, zonas de atención y por qué confiar en tu negocio." />
        <p className="text-right text-xs text-slate-400 mt-0.5">{form.business_description.length}/1200</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <input type="url" value={form.business_website} onChange={event => updateField('business_website', event.target.value)} className="border border-slate-300 rounded-xl px-3 py-2.5 text-sm" placeholder="Sitio web" />
        <input value={form.business_address} onChange={event => updateField('business_address', event.target.value)} maxLength={255} className="border border-slate-300 rounded-xl px-3 py-2.5 text-sm" placeholder="Dirección" />
        <input type="tel" value={form.business_phone} onChange={event => updateField('business_phone', event.target.value)} maxLength={20} className="border border-slate-300 rounded-xl px-3 py-2.5 text-sm" placeholder="Teléfono negocio" />
        <input type="tel" value={form.business_whatsapp} onChange={event => updateField('business_whatsapp', event.target.value)} maxLength={20} className="border border-slate-300 rounded-xl px-3 py-2.5 text-sm" placeholder="WhatsApp negocio" />
      </div>
      ...
      <div className="space-y-2">
        ...
        {form.business_hours.map((item, index) => (
          <div key={item.day} className="grid grid-cols-[1fr_auto_auto_auto] gap-2 items-center text-sm">
            ...
            <input type="time" disabled={item.closed} value={item.open || ''} onChange={event => updateHour(index, 'open', event.target.value)} className="border border-slate-300 rounded-lg px-2 py-1.5 disabled:bg-slate-100" />
            <input type="time" disabled={item.closed} value={item.close || ''} onChange={event => updateHour(index, 'close', event.target.value)} className="border border-slate-300 rounded-lg px-2 py-1.5 disabled:bg-slate-100" />
