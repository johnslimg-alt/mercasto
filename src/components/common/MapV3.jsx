...
function createPopupElement(ad, marker) {
  ...
  const filterPanelContent = showFilters ? (
    ...
    >
      ...
      <div className="map-filter-scroller min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain p-3.5 sm:p-4">
      ...
      <div className={`relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-md dark:border-slate-700 dark:bg-slate-950 ${className}`}>
        ...
        {showFullscreen && leaflet && !loadFailed && (
          <button
            ...
            disabled={locating}
            className="absolute bottom-3 left-3 z-[5] inline-flex min-h-12 min-w-12 items-center justify-center gap-1.5 rounded-full bg-blue-600 px-3.5 py-2.5 text-xs font-black text-white shadow-lg hover:bg-blue-500 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            title={t('map.nearMe')}
          >
            {locating ? (
