...
export default function ContactButton({ ad, user, t = {}, className = '' }) {
  ...
  const logContact = async (channel) => {
    try {
      ...
      // Contact analytics must never block the seller contact action.
    }
  };
  ...
  const sellerInitial = String(ad?.user?.name || t.ct_seller_fallback || '').trim().charAt(0).toUpperCase();
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 id="contact-dialog-title" className="font-bold text-lg text-gray-900 dark:text-white truncate">
                      {ad?.user?.name || t.ct_seller_fallback}
                    </h3>
                    {isVerified && (
