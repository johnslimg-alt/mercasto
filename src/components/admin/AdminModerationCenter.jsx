  ...
  return (
    <div className="space-y-5">
      ...
      <section className="sticky bottom-0 rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-xl backdrop-blur dark:border-slate-700 dark:bg-slate-900/95">
        <label htmlFor="admin-moderation-comment" className="block text-xs font-extrabold uppercase tracking-wide text-slate-500">{t('commentLabel')}</label>
        <textarea id="admin-moderation-comment" value={reason} onChange={(event) => setReason(event.target.value)} rows={3} placeholder={t('commentPlaceholder')} className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:border-lime-400 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
