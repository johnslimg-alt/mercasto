...
// --- ЛОГОТИП И ИКОНКИ ---
const MercastoLogo = ({ className = "h-11", isFooter = false, tagline = "Clasificados AI" }) => (
  <div className={`flex items-center gap-2 ${className}`}>
    ...
    <div className="flex flex-col justify-center">
      ...
      <span className={`mt-1 whitespace-nowrap text-[6px] font-bold uppercase leading-none tracking-[0.08em] sm:text-[7.5px] sm:tracking-widest ${isFooter ? 'text-[#84CC16]' : 'text-[#3f6212] dark:text-[#84CC16]'}`}>
        {tagline}
      </span>
    </div>
  </div>
...
// иначе при любом обновлении профиля/баланса React будет рвать и заново создавать сокет-соединение
  ...
  const renderAdCard = (ad, options = {}) => {
    ...
    return (
      <article ref={isCatalogFiller ? null : (node) => observeAdImpression(node, ad.id)} key={ad.id} onClick={() => handleViewAd(ad)} className={`market-card ad-result-card overflow-hidden cursor-pointer group flex flex-col h-full min-h-[252px] shrink-0 dark:border-slate-800 ${isHighlighted ? 'ring-2 ring-lime-400/70 shadow-lime-500/20' : ''}`}>
        ...
        <div className="max-w-[1440px] mx-auto px-4 lg:px-6">
          <div className="relative flex items-center gap-2 h-[44px] sm:h-[48px] lg:h-[54px]">
            <a href="/" onClick={(e) => { e.preventDefault(); setCurrentTab('home'); setViewedAd(null); setViewedCompany(null); setActiveCat(''); setSearchQuery(''); navigate('/'); }} className="flex items-center gap-2.5 shrink-0 hover:opacity-90 transition-opacity">
              <MercastoLogo className="h-6 sm:h-7 lg:h-9" tagline={t.ai_brand_short || 'Clasificados AI'} />
            </a>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            <div>
              <div className="footer-logo flex items-center gap-2 mb-3 h-8 opacity-80 hover:opacity-100 transition-opacity cursor-pointer" onClick={() => { setCurrentTab('home'); setViewedAd(null); setActiveCat(''); setSearchQuery(''); }}>
                <MercastoLogo className="h-8" isFooter={true} tagline={t.ai_brand_short || 'Clasificados AI'} />
              </div>
