...
// иначе при любом обновлении профиля/баланса React будет рвать и заново создавать сокет-соединение
  ...
  const renderAdDetailScreen = () => <AdDetailScreen ad={viewedAd} API_URL={API_URL} getImageUrl={getImageUrl} getImageUrls={getImageUrls} getCatName={getCatName} t={t} lang={lang} favoriteIds={favoriteIds} categoriesData={categoriesData} sliderAutoplay={sliderAutoplay} handleShareAd={handleShareAd} handleToggleFavorite={handleToggleFavorite} setReportingAd={setReportingAd} setShowReportModal={setShowReportModal} handleViewCompany={handleViewCompany} handleWhatsAppClick={handleWhatsAppClick} allAds={allAds} setViewedAd={setViewedAd} onBack={handleAdBack} MediaSlider={MediaSlider} renderAdCard={renderAdCard} AdSenseBanner={AdSenseBanner} currentUser={user} handleRenewAd={handleRenewAd} setAuthMode={setAuthMode} setShowAuthModal={setShowAuthModal} searchLocationInput={searchLocationInput} selectedState={selectedState} />;
    ...
    <div className="w-full min-h-screen bg-[var(--paper)] font-sans text-[var(--ink)] selection:bg-[#84CC16]/20">
      ...
      {appToast && (
        <div
          ...
          className={`fixed ${!viewedAd && currentTab !== 'post' ? 'bottom-[96px]' : 'bottom-6'} right-6 z-[9999] px-4 py-3 rounded-xl shadow-lg text-white text-sm font-medium transition-all max-w-xs md:bottom-6 ${appToast.type === 'error' ? 'bg-red-500' : 'bg-[#25D366]'}`}
        >
          {appToast.message}
        </div>
      )}
