...
For high-impact work use these roles: Analyst (best solution from evidence), Critic (find flaws and regressions), Fact checker (verify claims against code/tests/logs/authoritative sources), Engineer (implementation, rollback, observability, cost), and Red team (strongest counterexample/alternative). The parent/Chairman synthesizes only after the independent round and cross-review; evidence beats majority vote, and consensus is not treated as proof.
